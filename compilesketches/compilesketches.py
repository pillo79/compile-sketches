...
class CompileSketches:
    ...
    def compile_sketches(self):
        ...
        for sketch in sketch_list:
            ...
            compilation_result = self.compile_sketch(sketch_path=sketch,
                                                     clean_build_cache=self.enable_warnings_report or self.enable_issues_report)
    ...
    def compile_sketch(self, sketch_path, clean_build_cache):
        ...
        if not CompilationResult.success:
            print(f"::error::Compilation failed for {path_relative_to_workspace(path=sketch_path)}")
        else:
            time_summary = ""
            if diff_time > 60:
                if diff_time > 360:
    ...
    def get_sketch_report(self, compilation_result):
        ...
        if self.enable_issues_report:
            sketch_report[self.ReportKeys.issues] = self.get_issues_from_output(
    ...
    def get_issues_from_output(self, compilation_result):
        ...
        """
        issues = []
        compiler_issue_regex = re.compile("(:[0-9]+:[0-9]+|^\\S+): (fatal )?(warning|error):")
        linker_issue_regex = re.compile(": undefined reference to")

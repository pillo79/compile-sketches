...
def test_install_libraries(mocker, libraries, expected_manager, expected_path, expected_repository, expected_download):
    ...
    if len(expected_manager) > 0:
        compile_sketches.install_libraries_from_library_manager.assert_called_once_with(
            compile_sketches, library_list=expected_manager
        )
    else:
    ...
    if len(expected_path) > 0:
        compile_sketches.install_libraries_from_path.assert_called_once_with(
            compile_sketches, library_list=expected_path
        )
    else:
    ...
    if len(expected_repository) > 0:
        compile_sketches.install_libraries_from_repository.assert_called_once_with(
            compile_sketches, library_list=expected_repository
        )
    else:
    ...
    if len(expected_download) > 0:
        compile_sketches.install_libraries_from_download.assert_called_once_with(
            compile_sketches, library_list=expected_download
        )
    else:
...
def test_install_from_path(
    ...
    if exists != "no" and not force:
        ...
        assert capsys.readouterr().out.strip() == (
            "::error::Installation already exists: " + str(expected_destination_path)
        )
    else:
        compile_sketches.install_from_path(
            source_path=source_path,
...
def test_compile_sketch(capsys, mocker, returncode, expected_success):
    ...
    expected_stdout = (
        ...
        + str(compilesketches.path_relative_to_workspace(path=sketch_path))
        + "\n"
        + str(stdout)
        + "\n"
        + "::endgroup::"
...
def test_get_sketch_report(mocker, enable_warnings_report, do_deltas_report):
    ...
    # noinspection PyUnresolvedReferences
    compilesketches.CompileSketches.do_deltas_report.assert_called_once_with(
        compile_sketches,
...
def test_checkout_deltas_base_ref(monkeypatch, mocker):
    ...
    git.Repo.assert_called_once_with(path=os.environ["GITHUB_WORKSPACE"])
    Repo.fetch.assert_called_once_with(
        refspec=deltas_base_ref,
    ...
    if previous_size is None:
        ...
        if expected_relative_delta == compilesketches.CompileSketches.not_applicable_indicator:
            ...
            assert capsys.readouterr().out.strip() == (
                ...
                + size_name
                + ": "
                + str(expected_absolute_delta)
                + " ("
                + str(expected_relative_delta)
                + "%)"
            )
...
def test_get_sketches_report(monkeypatch, mocker):
    ...
    mocker.patch(
        "compilesketches.CompileSketches.get_warnings_summary_report",
        autospec=True,
    ...
    compile_sketches.get_sizes_summary_report.assert_called_once_with(
        compile_sketches, sketch_report_list=sketch_report_list
    )

    # noinspection PyUnresolvedReferences
    compile_sketches.get_warnings_summary_report.assert_called_once_with(
        compile_sketches, sketch_report_list=sketch_report_list
    )
...
def test_create_sketches_report_file(monkeypatch, tmp_path):
    ...
    compile_sketches = get_compilesketches_object(
        sketches_report_path=str(sketches_report_path), fqbn_arg="arduino:avr:uno"
    )
...
@pytest.mark.parametrize(
    "arcname, source_path, destination_name, expected_destination_name, expected_success",
    [
...
def test_install_from_download(
    ...
    if expected_success:
        compile_sketches.install_from_download(
            url=url,
            source_path=source_path,
            destination_parent_path=destination_parent_path,
            destination_name=destination_name,
        ...
        assert directories_are_same(
            left_directory=url_source_path.joinpath(source_path),
...
@pytest.mark.parametrize(
    "archive_extract_path, expected_archive_root_path",
    [
        (
...
def test_install_from_repository(mocker, url, source_path, destination_name, expected_destination_name):
    ...
    compile_sketches.install_from_repository(
        url=url,
        git_ref=git_ref,
        source_path=source_path,
        destination_parent_path=destination_parent_path,
    ...
    tempfile.mkdtemp.assert_called_once_with(
        dir=compile_sketches.temporary_directory.name, prefix="install_from_repository-"
    )
    compile_sketches.clone_repository.assert_called_once_with(
        compile_sketches, url=url, git_ref=git_ref, destination_path=clone_path
    )
    # noinspection PyUnresolvedReferences
